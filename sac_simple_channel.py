...
class SQNet(nn.Module):
    ...
    def __init__(self, num_inputs, num_actions, hidden_size=128, conv_channels=6, kernel_size=3, size_1=32, size_2=64, size_3=32):
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        x_aug = F.relu(self.fc2(x_aug))

        x_aug = F.relu(self.fc3(x_aug))
...
def run_sac(env, obs_state, num_actions, hyperps, device=torch.device("cuda"), render=True, metrified=True, save_dir='./'):
    ...
    memory = BasicBuffer(25)

    #hyperps['batch_size'] = min(24, hyperps['batch_size'])
    ...
    for epi in range(hyperps['max_epochs']):
        ...
        for step_numb in range(hyperps['max_steps']):
            ...
            w_vel, w_t, w_dis, w_col, w_lan = 0.5, -2, 5, 1, 1
            ...
            #all_actions, all_pol_stats, all_stds, all_means, all_rewards, all_scenario_wins_rewards
            #obs, steps, wall_street, all_actions, all_pol_stats, all_stds, all_means, all_rewards, all_scenario_wins_rewards
            
            #if(total_steps % 80 > 40):
