...
writer = SummaryWriter('runs/sac_a3c_exp1')
...
def train_on_env(env_name, rank, lock, shared_model, shared_optimizer, obs_state, num_actions, hyperps, shared_mem, device=torch.device("cpu"), render=True):
    ...
    env = env_name.fromrank(rank, withlock=True)
    env._env.lock = lock
        ...
        for step_numb in range(hyperps['max_steps']):
            ...
            if len(memory) > hyperps['batch_size']*2 and total_steps % hyperps['update_every'] == 0:
                for i in range(hyperps['updates_per_step']):
                    ...
                    critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = sac_agent.update(memory, updates, shared_model, shared_optimizer)
                    writer.add_scalar('Loss{}/critic_1'.format(rank), critic_1_loss, total_steps)
                    writer.add_scalar('Loss{}/critic_2'.format(rank), critic_2_loss, total_steps)
                    writer.add_scalar('Loss{}/policy'.format(rank), policy_loss, total_steps)
                    writer.add_scalar('Loss{}/ent'.format(rank), ent_loss, total_steps)
                    writer.add_scalar('Loss{}/alpha'.format(rank), alpha, total_steps)
