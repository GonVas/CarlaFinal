...
if(args.download):
    import gdown
    import tarfile
    url_small = 'https://drive.google.com/uc?id=1F0la2wOVRJ7htAknQRb607eNJjqYQEk1&export=download'
    url = 'https://drive.google.com/uc?id=1ukLghVAbVx3ocfH8RJoNIQT3X8H082MW&export=download'
    output = 'carla_data_0.tar.gz'
    output_small = 'carla_data_2.tar.gz'
    gdown.download(url, output, quiet=False)
    tar = tarfile.open(output)
    tar.extractall(path="./carla_data_0/")
    tar.close()
    filelist = glob('./carla_data_0/*.png')
else:
    #filelist = glob('./carla_data_small/*.png')
    filelist = glob('./data/carla_data_1/*.png')
